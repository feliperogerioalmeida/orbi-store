...
const UpgradeSimulator = () => {
  ...
  const calculateValue = () => {
    ...
    let baseValue = selectedCapacity.maxUpgradeValue;

    if (batteryHealth === "<80%") baseValue -= 500;
    if (batteryHealth === "80%-85%") baseValue -= 300;

    if (issues.includes("Trincado")) baseValue -= 700;
    if (issues.includes("Face ID Defeituoso")) baseValue -= 800;
    if (issues.includes("Vidro Traseiro Quebrado")) baseValue -= 500;
  ...
  const sendToWhatsApp = () => {
    const message = `Olá! Gostaria de continuar com a simulação de upgrade do meu iPhone.\n
...
Valor Estimado: R$ ${estimatedValue}`;
  ...
  return (
    <div className="pt-[120px] text-white px-4 h-full w-full min-h-screen flex flex-col gap-8 bg-custom-bg-2">
      <h1 className="text-3xl font-bold text-center">Simulação de Upgrade</h1>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {[
                  ...
                  "Flash Não Funciona",
                  "Bateria Trocada",
                  "Já Foi Para A Assistência Tecnica",
                ].map((issue) => (
